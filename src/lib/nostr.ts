...
class NostrHelpers {
  ...
  public derivePublicKey(privateKey: string): string {
    try {
      ...
      logger.logger.error('Error deriving public key', { 
  ...
  public async getNostrPublicKey(): Promise<string | null> {
    try {
      ...
      console.logger.error('Timeout or error while waiting for Nostr extension:', error);
      return null;
    }
  }
  ...
  public async getUserPublicKey(): Promise<string> {
    try {
      ...
      logger.logger.error('Error getting Nostr public key', { 
        error: error instanceof Error ? error.message : 'Unknown error' 
      });
      throw error;
    }
  ...
  public async signEvent(event: NostrEvent): Promise<NostrEvent> {
    try {
      ...
      logger.logger.error('Error signing Nostr event', { 
        error: error instanceof Error ? error.message : 'Unknown error' 
      });
      throw error;
    }
  ...
  public async enableTestMode(): Promise<void> {
    try {
      ...
      logger.logger.error('Error enabling test mode', { 
        error: error instanceof Error ? error.message : 'Unknown error' 
      });
      throw error;
    }
  ...
  public async disableTestMode(): Promise<void> {
    try {
      ...
      logger.logger.error('Error disabling test mode', { 
        error: error instanceof Error ? error.message : 'Unknown error' 
      });
      throw error;
    }
  ...
  public generateTestKeyPair(): TestKeypair {
    try {
      ...
      try {
        ...
        logger.logger.error('Error generating test keypair', { 
...
export function verifyNostrSignature(pubkey: string, signature: string, challenge: string): boolean {
  try {
    ...
    logger.logger.error('Error verifying Nostr signature', {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
    return false;
  }
...
export function generateRegularAccount(): TestKeypair {
  try {
    ...
    try {
      ...
      logger.logger.error('Error generating regular keypair', { 
        error: cryptoError instanceof Error ? cryptoError.message : 'Unknown error' 
      });
    }
    ...
    logger.logger.error('Error in generateRegularAccount', {
      error: error instanceof Error ? error.message : 'Unknown error'
    });
