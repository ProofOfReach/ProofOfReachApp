...
// Import the mocked formatUserErrorMessage function
...
const ApiErrorDisplay: React.FC<ApiErrorDisplayProps> = ({
  ...
  const errorMessage = formatUserErrorMessage(error, message);
  
  // Handle status code in the title if available
  const statusTitle = error.status ? `${title} (${error.status})` : title;
