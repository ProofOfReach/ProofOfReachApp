  ...
  useEffect(() => {
    ...
    const checkFrequencyCap = async () => {
      try {
        ...
        if (!response.ok) {
          console.log('Failed to check frequency cap:', response.statusText);
          return;
        }
        ...
        console.log('Error checking frequency cap:', error);
      }
    };
  ...
  const handleAdViewed = useCallback((adId: UserRole, advertiserName: string) => {
    ...
    if (isTestMode && typeof window !== 'undefined') {
      try {
        ...
        console.log(message, { duration: 4000 }); // Slightly longer duration for better visibility
