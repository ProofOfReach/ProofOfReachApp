  ...
  const fetchRolesFromCache = () => {
    ...
    if (cachedRoles) {
      try {
        ...
        logger.logger.error('Error parsing cached roles:', error);
    ...
    try {
      ...
      if (isTestModeActive) {
        ...
        try {
          ...
          logger.logger.error('Error storing roles in enhanced storage:', error);
          // Fallback to localStorage for backward compatibility
      ...
      try {
        ...
        if (response.ok) {
          ...
          logger.logger.error('Failed to fetch available roles:', await response.text());
        ...
        logger.logger.error('Error fetching roles from API:', error);
        // Fallback to user role only for security reasons
        setAvailableRoles(['viewer']);
        ...
        try {
          ...
          logger.logger.error('Error getting roles from RoleManager:', fallbackError);
          notifyRolesUpdated(['viewer'], currentRole);
        }
      }
    } catch (error) {
      logger.logger.error('Error in role loading process:', error);
      setAvailableRoles(['viewer']);
      notifyRolesUpdated(['viewer'], currentRole);
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      if (isTestModeActive) {
        ...
        try {
          ...
          logger.logger.error('Error using enhanced storage system:', storageError);
      ...
      if (success) {
        ...
        logger.logger.error('Failed to change role');
