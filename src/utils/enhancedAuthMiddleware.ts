...
export async function authenticateRequest(req: NextApiRequest): Promise<AuthenticatedUser> {
  try {
    ...
    const roles: UserRoleType[] = ['user']; // Base user role is always available
...
export const enhancedAuthMiddleware = (
  handler: (req: NextApiRequest, res: NextApiResponse, user: AuthenticatedUser) => Promise<any>,
...
export const requireRoles = (requiredRoles: UserRoleType[]) => {
  return (handler: (req: NextApiRequest, res: NextApiResponse, user: AuthenticatedUser) => Promise<any>) => {
