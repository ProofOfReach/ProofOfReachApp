...
export default async function handleProfileRequest(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.query.pubkey) {
      ...
      if (targetPubkey.startsWith('npub1')) {
        try {
          ...
          logger.logger.error('Failed to decode npub:', targetPubkey, decodeError);
    ...
    logger.logger.error('Error fetching Nostr profile:', error);
...
async function fetchProfileFromRelay(pubkey: string): Promise<any> {
  ...
  try {
    ...
    try {
      ...
      logger.logger.error('Error parsing profile content:', e);
      return null;
    }
  } catch (error) {
    logger.logger.error('Error fetching from relays:', error);
    logger.logger.error('Error details:', error instanceof Error ? error.message : String(error));
    pool.close(RELAYS);
