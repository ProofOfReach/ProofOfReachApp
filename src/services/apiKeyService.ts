...
export async function createApiKey(params: ApiKeyCreateParams): Promise<ApiKeyResponse> {
  ...
  try {
    ...
    logger.log(`Failed to create API key: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
export async function getUserApiKeys(userId: string) {
  try {
    ...
    logger.log(`Failed to get API keys for user: ${error instanceof Error ? error.message : 'Unknown error'}`);
...
export async function verifyApiKey(apiKeyValue: string) {
  ...
  try {
    ...
    logger.log(`Error verifying API key: ${error instanceof Error ? error.message : 'Unknown error'}`);
